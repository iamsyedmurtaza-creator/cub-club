...
export default function MobileBottomNav() {
  ...
  const { count } = useCart();
  const items = [{ to: "/", label: "Home", icon: Home }, { to: "/shop", label: "Shop", icon: PackageSearch }, { to: "/cart", label: "Cart", icon: ShoppingBag, count }, { to: user ? (isAdmin ? "/admin" : "/my-orders") : "/login", label: isAdmin ? "Admin" : "Account", icon: isAdmin ? LayoutDashboard : UserRound }];
